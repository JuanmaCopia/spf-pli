...
public class HashMapHMap {
     ...
     *         the specified key.
     */
    public HashMapStr put(int key, HashMapStr value) {
        int hash = hash(key);
        int i = indexFor(hash, DEFAULT_INITIAL_CAPACITY);

        for (Entry e = getTable(i); e != null; e = e.next) {
            if (e.hash == hash && eq(key, e.key)) {
                HashMapStr oldValue = e.value;
                e.value = value;
                e.recordAccess(this);
                return oldValue;
            }
     ...
     *         specified key.
     */
    public HashMapStr remove(int key) {
     ...
     */
    void addEntry(int hash, int key, HashMapStr value, int bucketIndex) {
     ...
     * readObject.
     */
    void createEntry(int hash, int key, HashMapStr value, int bucketIndex) {
        setTable(bucketIndex, new Entry(hash, key, value, getTable(bucketIndex)));
        size++;
    }
    ...
    public static class Entry {
         ...
         * Create new entry.
         */
        Entry(int h, int k, HashMapStr v, Entry n) {
            value = v;
            next = n;
            key = k;
            hash = h;
        ...
        public HashMapStr getValue() {
            return value;
        }

        public HashMapStr setValue(HashMapStr newValue) {
            HashMapStr oldValue = value;
            value = newValue;
            return oldValue;
        }
         ...
         */
        void recordAccess(HashMapHMap m) {
        }

        /**
         * This method is invoked whenever the entry is removed from the table.
         */
        void recordRemoval(HashMapHMap m) {
        }
    }
