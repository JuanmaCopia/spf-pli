...
public class Helper {
    ...
    // public static SymbolicInteger SymbolicNull = new
    // SymbolicInteger("SymbolicNull"); // hack for handling static fields; may no
    // longer need it
    ...
    public static Expression initializeInstanceField(FieldInfo field, ElementInfo eiRef, String refChain, String suffix,
        ...
        if (field instanceof IntegerFieldInfo || field instanceof LongFieldInfo) {
        	...
        	/*
            ClassInfo cls = eiRef.getClassInfo();
            String simpleClassName = cls.getSimpleName();
            String fieldSig = simpleClassName + "." + name;
            if (SymSolveHeapSolver.minMaxMap.containsKey(fieldSig)) {
                IntSet intset = SymSolveHeapSolver.minMaxMap.get(fieldSig);
                int min = intset.getMin();
                int max = intset.getMax();
                sym_v = new SymbolicInteger(fullName, min, max);
            } else {
                sym_v = new SymbolicInteger(fullName);
            }
            */
            ...
            // treat boolean as an integer with rimport gov.nasa.jpf.symbc.heap.SymbolicInputHeap;ange [0,1]
    ...
    public static Expression initializeStaticField(FieldInfo staticField, ClassInfo ci, ThreadInfo ti, String suffix) {
    ...
    public static void initializeStaticFields(FieldInfo[] staticFields, ClassInfo ci, ThreadInfo ti) {
    ...
    public static HelperResult addNewArrayHeapNode(ClassInfo typeClassInfo, ThreadInfo ti, Object attr,
            PathCondition pcHeap, SymbolicInputHeap symInputHeap, int numSymRefs, HeapNode[] prevSymRefs,
            boolean setShared, IntegerExpression indexAttr, int arrayRef) {
        int daIndex = ti.getHeap().newObject(typeClassInfo, ti).getObjectRef();
        ti.getHeap().registerPinDown(daIndex);
        String refChain = ((ArrayExpression) attr).getName(); // + "[" + daIndex + "]"; // do we really need to add
                                                              // daIndex here?
        SymbolicInteger newSymRef = new SymbolicInteger(refChain);
        ElementInfo eiRef = ti.getModifiableElementInfo(daIndex);// ti.getElementInfo(daIndex); // TODO to review!
        if (setShared) {
